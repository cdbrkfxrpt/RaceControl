...
def data():
    """
    The data() method returns a template rendered by Flask/Jinja2 based on
    templates/data.html.
    """
...
def subscribe():
    """
    The subscribe() method returns a Flask.Response object, which contains data
    in the form of a 'data: some_data\n\n' generated by the gen() method, which
    in turn gets messages from the webcom queue and yields them. The Response
    object also flags the data as 'text/event-stream' mimetype for the browser
    to identify. The subscribe() method is called by the JavaScript script in
    static/js/raceflot.js, where it is attached to an EventSource, which is the
    JavaScript implementation of a server side event object.
    """
    def gen():
